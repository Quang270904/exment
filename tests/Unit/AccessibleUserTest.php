        ...
        foreach ($events as $event) {
            ...
            if ( isset($event['event']['type']) && in_array($event['event']['type'], ['exception', 'occurrence']) ) {
                if ( $event['event']['type'] == 'occurrence' ){
                    ...
                    // 終日対応
                    if ( $master_event[0]['event']['isAllday'] ){
                    ...
                    // 1. [開始日時]、[終了日時]、[開催者のメールアドレス]で検索
                    //   ⇒ occurrence で登録済みのレコードがある場合、それを更新するため
                    // 2-1. 1.で該当レコードがあれば、そちらを更新。なければ、                    
                    // 2-2. 1.で該当レコードがなければ、iCalUId or event_id で検索
                    ...
                    if ( isset($records) && $records->count() > 0 ) {
                        ...
                        if ( array_key_exists('iCalUId', $event['event']) && !is_nullorempty($event['event']['iCalUId']) ){
                            $record = $exment_schedule->getValueModel()
                                ->Where($searchColumn1->getIndexColumnName(), '=', $event['event']['iCalUId'])
                                ->first();
                        } else {
                            // TODO: 不要なら削除
                            $child_record = $exmen_attendance_status->getValueModel()
                ...
                If (isset($record)) {
                    if ( $event['event']['isCancelled'] ) {
                        ...
                        if ( $record->getValue('event_id') <> $event_id && $record->getValue('iCalUId') == $event['event']['iCalUId'] ){
                            ...
                            if ( is_nullorempty( $event['event']['organizer'] ) ){
                        ...
                        if ( !$event['event']['isOrganizer'] ) $event_id = null;
                        ...
                        if ( isset($event['status']) && count($event['status']) > 0 ) {
                            ...
                            GraphHelper::patchAttendanceStatus($attendance_status_custom_values, $event['status'], $record->id);    
                        }
                    }
                } else {
                    if ( $event['event']['isCancelled'] ) {
...
*/
     ...
     * @param array $status
     */
    public static function patchAttendanceStatus($attendance_status_custom_values, array $status, int $parent_id): void {
            ...
            if (!isset($attendance_status_custom_values[0])) {
                ...
                if (empty($childRecord)) {
                    ...
                    try {
                        ...
                        //$childRecord->setValue('series_master_event_id', $attendeeStatus['series_master_event_id']);
    ...
    private static function makeRecurrenceValue(Event $event, $start_time, $end_time){
        ...
        $value = [
            ...
            'pattern' => [
                ...
                'daysOfWeek' => $pattern->getDaysOfWeek()[0]->value(),
